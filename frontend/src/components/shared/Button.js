...
const Button = ({ children, className, variant = 'primary', size = 'md', loading = false, disabled, as = 'button', ...props }) => {
    ...
    const classes = clsx(baseClasses, variantClasses[variant], sizeClasses[size], {
        'opacity-50 cursor-not-allowed': disabled || loading,
        'cursor-pointer': !disabled && !loading,
    }, className);
    const content = (_jsxs(_Fragment, { children: [loading && _jsx(Loader, { className: "w-4 h-4 mr-2" }), children] }));
    if (as === 'a') {
        return (_jsx("a", { className: classes, ...props, children: content }));
    }
